...
void introduction(void)
{
    ...
    for(int i=0; i<=100; i++)
        printf("-");
    printf("\n");
    printf("\t\t\t\tSTUDENT RECORD SYSTEM FOR DEPARTMENT OF CSE\n");
    printf("\t\t\t\t\tPremier University, Chittagong.\n");
    printf("\t\t\t\t\t\tEstd. : 2002\n");
    for(int i=0; i<=100; i++)
        printf("-");
    printf("\n");
...
int get_password(int check){
    ...
    if(check>=5 && found!=1)
    {
        ...
        printf("\t\t\tYou are trying to login illegally.");
        exit(0);
    }
    if(found!=1)
    {
...
int conf_record(ll s_id){
    ...
    if(found){
        printf("\n\nThe record for the same ID is already in the database.\n");
        printf("The informations for the ID are given below\n\n\n");
